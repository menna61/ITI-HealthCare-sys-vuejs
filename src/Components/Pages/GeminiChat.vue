<template>
  <div class="flex h-screen bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-950">
    <!-- Sidebar for Chat History -->
    <div class="hidden md:flex md:w-1/4 bg-white/80 dark:bg-gray-800/80 backdrop-blur border-r border-gray-200 dark:border-gray-700 flex-col position-fixed">
      <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        <h2 class="text-base font-semibold text-gray-900 dark:text-white">Conversations</h2>
        <button @click="startNewChat" class="inline-flex items-center gap-2 bg-[var(--main-color-500)] text-white py-2 px-3 rounded-lg hover:bg-[var(--main-color-600)] transition-colors">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-2">
        <div v-if="chatHistory.length === 0" class="text-gray-500 dark:text-gray-400 text-sm">No previous chats</div>
        <div v-else>
          <div v-for="(chat, index) in chatHistory" :key="chat.id || index" class="relative group">
            <button @click="loadChat(chat)" class="w-full text-left p-3 bg-gray-50/80 dark:bg-gray-700/60 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-xl transition-colors">
              <p class="text-sm text-gray-900 dark:text-white truncate">{{ chat.messages && chat.messages[0] ? chat.messages[0].text : 'New Chat' }}</p>
              <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-1">{{ formatDate(chat.createdAt) }}</p>
            </button>
            <button @click.stop="deleteChat(index)" class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-red-500 hover:bg-red-600 text-white rounded-full p-1" title="Delete chat">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
    ...
    <div class="flex-1 flex flex-col">
      <!-- Header -->
      <div class="sticky top-0 z-10 border-b border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/70 backdrop-blur">
        <div class="px-4 md:px-6 py-3 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <img :src="aiIcon" alt="AI" class="w-8 h-8 rounded" />
            <div>
              <h1 class="text-sm md:text-base font-semibold text-gray-900 dark:text-white">Your AI assistant</h1>
              <div class="flex items-center gap-2">
                <span class="inline-flex h-2 w-2 rounded-full" :class="isLoading ? 'bg-yellow-500 animate-pulse' : 'bg-emerald-500'"></span>
                <span class="text-[11px] text-gray-500 dark:text-gray-400">{{ isLoading ? 'Thinking...' : 'Online' }}</span>
              </div>
            </div>
          </div>
          <button class="md:hidden inline-flex items-center gap-2 text-gray-700 dark:text-gray-300 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800" @click="startNewChat">New Chat</button>
        </div>
      </div>

      <div class="flex-1 flex">
        <!-- Chat Messages -->
        <div ref="messagesPane" class="flex-1 overflow-y-auto px-4 md:px-6 py-6 space-y-4">
          <!-- Empty state -->
          <div v-if="currentChat.messages.length === 0 && !isLoading" class="h-full w-full flex items-center justify-center">
            <div class="text-center">
              <img :src="startImg" alt="Start chatting" class="mx-auto w-40 h-40 object-contain opacity-90" />
              <h3 class="mt-4 text-lg font-semibold text-gray-900 dark:text-white">Start a conversation</h3>
              <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Ask anything about health, appointments, or general queries.</p>
            </div>
          </div>

          <!-- Messages list -->
          <div v-for="(message, index) in currentChat.messages" :key="index" class="flex items-end gap-3" :class="message.sender === 'user' ? 'justify-end' : 'justify-start'">
            <img v-if="message.sender !== 'user'" :src="aiIcon" alt="AI" class="w-8 h-8 rounded-md shadow-sm border border-gray-200 dark:border-gray-700" />
            <div :class="[
              ...
              <span class="block text-[11px] mt-1 opacity-70">{{ formatDate(message.createdAt || new Date()) }}</span>
            </div>
          </div>

          <!-- Typing indicator -->
          <div v-if="isLoading" class="flex items-end gap-3 justify-start">
            <img :src="aiIcon" alt="AI" class="w-8 h-8 rounded-md shadow-sm border border-gray-200 dark:border-gray-700" />
            <div class="bg-white/90 dark:bg-gray-800/80 text-gray-900 dark:text-gray-50 border border-gray-200/70 dark:border-gray-700/60 rounded-2xl rounded-bl-md px-4 py-3 shadow-sm">
        ...
        <aside class="hidden xl:flex xl:w-80 flex-col border-l border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-900/70 p-4 gap-3">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-white">AI Medical Tools</h3>
          <p class="text-xs text-gray-500 dark:text-gray-400">Select text in chat or type a message, then click a tool.</p>



          <button class="w-full text-left py-3 px-3 rounded-lg bg-white dark:bg-gray-800 border hover:shadow-sm" @click="runFeature('diagnose')">
          ...
          <button class="w-full text-left py-3 px-3 rounded-lg bg-white dark:bg-gray-800 border hover:shadow-sm" @click="runFeature('interactions')">
            <div class="flex items-center justify-between">
              <div>
          ...
          <button @click="runPatientExplanation" class="w-full py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm">
      ...
      <!-- Input Area -->
      <div class="border-t border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/70 backdrop-blur">
        <div class="px-4 md:px-6 py-4">
          <div class="flex items-end gap-2">
            <textarea v-model="newMessage" @keyup.enter.exact.prevent="sendMessage" rows="1" placeholder="Type your message..." class="flex-1 resize-none max-h-40 min-h-[48px] p-3 border border-gray-300 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--main-color-500)] focus:border-transparent" />
            <button @click="sendMessage" :disabled="isLoading || !newMessage.trim()" class="inline-flex items-center justify-center gap-2 bg-[var(--main-color-500)] text-white px-5 py-3 rounded-xl hover:bg-[var(--main-color-600)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
...
export default {
  ...
  methods: {
    ...
    async getGeminiResponse() {
      ...
      if (!apiKey) throw new Error('Set your Gemini API key in localStorage: localStorage.setItem("gemini_api_key", "YOUR_KEY")');
      ...
      if (history) {
        try {
          this.chatHistory = JSON.parse(history).map((chat) => ({
            ...
            messages: (chat.messages || []).map((m) => ({ ...m, createdAt: m.createdAt ? new Date(m.createdAt) : new Date() })),
          }));
        } catch (e) {
    ...
    async runFeature(feature) {
      ...
      try {
        ...
        const lastIndex = this.currentChat.messages.map((m) => m.text).lastIndexOf(`${this.humanFeatureName(feature)}: Running...`);
        if (lastIndex >= 0) this.currentChat.messages.splice(lastIndex, 1);
        this.currentChat.messages.push({ sender: "ai", text: `Error running ${this.humanFeatureName(feature)}: ${err.message}`, createdAt: new Date() });
      } finally {
        this.isLoading = false;
        this.saveCurrentChat();
        this.scrollToBottom();
...
`;
        ...
        const lastIndex = this.currentChat.messages.map((m) => m.text).lastIndexOf(runningLabel);
        if (lastIndex >= 0) this.currentChat.messages.splice(lastIndex, 1);
        this.currentChat.messages.push({ sender: "ai", text: `Error explaining to patient: ${err.message}`, createdAt: new Date() });
      } finally {
        this.isLoading = false;
        this.saveCurrentChat();
        this.scrollToBottom();
    ...
    async callGeminiWithPrompt(promptText) {
      const apiKey = localStorage.getItem("gemini_api_key") || window.__GEMINI_API_KEY__ || "";
      if (!apiKey) throw new Error('Set your Gemini API key in localStorage: localStorage.setItem("gemini_api_key", "AIzaSyAwD5bS77vogG-DqDltTYv5tnpq_fFqWRE")');
